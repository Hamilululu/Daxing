<template>
  <div class="table-content">
    ...
    <el-table :row-class-name="tableRowClassName" ref="singleTable" :data="tableDataList" highlight-current-row
              :max-height="MH" style="width: 100%">
      <el-table-column property="key" label="点位" header-align="center" width="60">
        <template slot-scope="scope">
          <div style="text-align: center;font-size: 22px;overflow: auto">
            {{ scope.row.key }}
          </div>
        </template>
      </el-table-column>
      <el-table-column header-align="center" label="波形">
        <template slot-scope="scope">
          <div>
            <d-chart1 :height="height" v-on:changeLoading="loadingType($event)" :ids="scope.row.key"
                      :options="scope.row.pointData" :scopeData="scope.row.maxValue" :changeIds="changeId"
                      :flagLoaing="loading">
            </d-chart1>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="峰值" header-align="center" width="60">
        <template slot-scope="scope">
      ...
      <!--        width="48">-->
      <!--      </el-table-column>-->
      <el-table-column header-align="center" property="idStr" label="状态" width="45">
        <template slot-scope="scope">
          <div style="text-align: center">
            <i v-if="!scope.row.health" style="color: orange;font-size: 12px" class="el-icon-warning"></i>
...
<script>
  ...
  export default {
    ...
    props: ['socketID', 'mHeight', 'sTime', 'eTime', 'form', 'sign', 'height'],
    watch: {
      height: {
        handler (n, o) {
          // this.form = n
    ...
    mounted () {
      this.$nextTick(() => {
       ...
       // this.createCurrentPt()
        ...
        // this.flushWaveDataTimer=setInterval(()=>{
        //   this.setDataPoint()
        // },10)
        // this.setDataPoint()
      })
    ...
    computed: {
      ...
      //    return this.$store.state.d2admin.waveBuffer.waveTableBuf;
      // },
      // setDataPoint () {
      //   let that = this
    ...
    methods: {
      // getSeiWaveData () {
      //     let siteMark = this.tableDataList[0].key
      //     // 往前推迟3分钟
      //     let timeStamp = Date.parse(new Date()) - 15 * 60 * 1000
      //     let data = {
      //       'currTime': timeStamp,
      //       'siteMark': siteMark,
      //       'chn': 'x',
      //       'pageSize': this.pointDataLen //分页大小
      //     }
      //     postUserList('/on/wave/seis/range', data).then(res => {
      //       //返回值
      //       let pointData = res.data.waveData.pointData
      //       // 创建的DOM对象
      //       let data = JSON.parse(JSON.stringify(this.tableDataList[1].pointData))
      //       for (let j = 0; j < pointData.length; j++) {
      //         let json = {
      //           name: pointData[j].countValue,
      //           value: [pointData[j].timestamp, pointData[j].galValue]
      //         }
      //         data.push(json)
      //         this.tableDataList[0].pointData = data
      //       }
      //     })
      //
      //
      ...
      createObj () {
        let obj = {}
        for (let l = 0; l < 35; l++) {
          let key = l > 10 ? 'DX' + l : 'DX0' + l
          obj[key] = null
        }
        this.dataObj = obj
      },
      // getSeiWaveData () {
      //   // that.tableDataList.length 35
      //   for (let i = 0; i < this.tableDataList.length; i++) {
      //     // 获取 点位 DX01
      //     let siteMark = this.tableDataList[i].key
      //     let currDataIndex = this.currPt[siteMark]
      //     // 往前推迟3分钟
      //     // let timeStamp = Date.parse(new Date()) - 30 * 60 * 1000
      //     let timeStamp = this.createObj[siteMark]
      //     let data = {
      //       'currTime': timeStamp,
      //       'siteMark': siteMark,
      //       'chn': 'x',
      ...
      //
      //       // 创建的DOM对象
      //       let data = JSON.parse(JSON.stringify(this.tableDataList[i].pointData))
      //       for (let j = 0; j < pointData.length; j++) {
      //         //获取返回值的时间戳
      //         let newTimestamp = pointData[j].timestamp
      //         this.createObj[siteMark] = newTimestamp
      //         let json = {
      //           name: currDataIndex,
      //           value: [currDataIndex, pointData[j].galValue]
      //         }
      //         data[j] = json
      //         this.tableDataList[i].pointData = data
      //       }
      ...
      //         let afterIndex = currDataIndex + k
      //         if (afterIndex < this.pointDataLen) {
      //           this.tableDataList[i].pointData[afterIndex] = {
      //             name: afterIndex,
      //             value: [afterIndex, null]
      //           }
      //         }
      //       }
      //
      //       if (this.currPt[siteMark] >= this.pointDataLen - 1) {
      //         this.currPt[siteMark] = 0
      //       } else {
      //         this.currPt[siteMark] += 1
      //       }
      //       // this.getSeiWaveData()
      //     })
      //     // console.log(this.tableDataList)
      ...
      mqttRecv () {
        ...
        //   clientId: '4861654643546',
        //   clean: true
        // }
        // client = mqtt.connect('ws://10.16.0.27:10082/mqtt', options)
        // client.on('connect', () => {
        //   client.subscribe('seis_wave', {
        //     qos: 2
        //   }, (error) => {
        //     console.log(error)
        //   })
      ...
      submit (par) {
        ...
        if (this.form.point) {
          ...
          this.websocket(this.form.axis.toLowerCase())
          for (let i = 0; i < this.tableDataList.length; i++) {
            let tableIndex = this.tableDataList[i].key.split('DX')[1] * 1
            if (index === tableIndex) {
              this.tableDataList = [this.tableDataList[i]]
            }
          }
...
<style scoped>
  ...
  /deep/ .el-pagination.el-pagination--small .btn-next,
  ...
  /deep/ .el-pagination.el-pagination--small .el-pager .el-pagination .btn-next .el-icon,
