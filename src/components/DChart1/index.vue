<template>
  <div style="width: 100%;" :style="`height:${height}px`">
    <div ref="chart" :id="ids" style="width: 100%;height: 100%;">
    </div>
  </div>
...
<script>
  ...
  export default {
    props: ['ids', 'options', 'changeIds', 'height'],
    ...
    methods: {
      ...
      sortDataChart (val) {
        ...
        for (let i = 0; i < arr.length; i++) {
          if (arr[i].value[1] === null) {
            this.sortMax = arr[i - 1].value[1]
            break
          }
          this.sortMin = arr[arr.length - 1].value[1]
        }
        // console.log(arr);
        this.sortMin = arr[0].value[1]
        // this.sortMin = arr[arr.length - 1].value[1]
...
// 如果宽高比大于 1 则宽度为 100，如果小于 1 则高度为 100，保证了不超过 100x100 的区域
        ...
        // })
        // elementResize.listenTo( this.dom, function(element) {
        //   alert(0)
        //   echarts.init( this.dom).resize() // 当元素尺寸发生改变是会触发此事件，刷新图表
        // });
      }
    }
  }
